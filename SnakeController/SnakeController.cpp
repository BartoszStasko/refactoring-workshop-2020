      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        m_map.MapDimension = std::make_pair(width, height);
        m_map.FoodPosition = std::make_pair(foodX, foodY);
...
{
    return x < 0 or y < 0 or x >= m_map.MapDimension.first or y >= m_map.MapDimension.second;
}

void Controller::sendPlaceNewFood(int x, int y)
{
    m_map.FoodPosition = std::make_pair(x, y);
...
void Controller::sendClearOldFood()
{
    ...
    clearOldFood.x = m_map.FoodPosition.first;
    clearOldFood.y = m_map.FoodPosition.second;
...
{
    if (std::make_pair(newHead.x, newHead.y) == m_map.FoodPosition) {
...
{
    //m_eventHandlerMap.at(e->getMessageId())->receive(std::move(e));
...
void Controller::registerEventHandler(std::uint32_t messageId, std::unique_ptr<IEventHandler>&& eventHandler)
{
    m_eventHandlerMap.emplace(messageId, std::move(eventHandler));
}
